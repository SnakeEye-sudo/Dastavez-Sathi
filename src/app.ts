(() => {
  ...
  type View = "capture" | "review" | "export";
  type Preset = "document" | "receipt" | "bw" | "color";
  ...
  const presetBase: Record<Preset, { b: number; c: number; gray: boolean; t?: number }> = {
  ...
  const refs = {
    ...
    openDrawerBtn: document.getElementById("openDrawerBtn") as HTMLButtonElement,
    closeDrawerBtn: document.getElementById("closeDrawerBtn") as HTMLButtonElement,
    ...
    reminderTime: document.getElementById("reminderTime") as HTMLInputElement,
    saveReminderBtn: document.getElementById("saveReminderBtn") as HTMLButtonElement,
    reminderStatus: document.getElementById("reminderStatus") as HTMLElement,
    tabs: document.getElementById("viewTabs") as HTMLElement,
    fileInput: document.getElementById("fileInput") as HTMLInputElement,
    importBtn: document.getElementById("importBtn") as HTMLButtonElement,
    cameraBtn: document.getElementById("cameraBtn") as HTMLButtonElement,
    stopCameraBtn: document.getElementById("stopCameraBtn") as HTMLButtonElement,
    captureBtn: document.getElementById("capturePhotoBtn") as HTMLButtonElement,
    video: document.getElementById("cameraPreview") as HTMLVideoElement,
    cameraState: document.getElementById("cameraState") as HTMLElement,
    strip: document.getElementById("pageStrip") as HTMLElement,
    stripEmpty: document.getElementById("pageStripEmpty") as HTMLElement,
    preview: document.getElementById("previewCanvas") as HTMLCanvasElement,
    previewEmpty: document.getElementById("previewEmpty") as HTMLElement,
    title: document.getElementById("titleInput") as HTMLInputElement,
    preset: document.getElementById("presetSelect") as HTMLSelectElement,
    brightness: document.getElementById("brightnessRange") as HTMLInputElement,
    ...
    pagesMetric: document.getElementById("pagesMetricValue") as HTMLElement,
    docsMetric: document.getElementById("docsMetricValue") as HTMLElement,
    modeMetric: document.getElementById("modeMetricValue") as HTMLElement
  };

  const state = {
    theme: (localStorage.getItem(store.theme) as Theme) || "ink",
    view: "capture" as View,
    pages: [] as Page[],
    current: "",
    docs: loadDocs(),
  ...
  function q<T>(selector: string) {
    return Array.from(document.querySelectorAll<T>(selector));
  }

  function setView(view: View) {
    state.view = view;
    q<HTMLElement>(".tool-view").forEach((el) => el.classList.toggle("active", el.id === `view-${view}`));
    renderTabs();
  }
  ...
  function renderTabs() {
    ...
    ["capture", "review", "export"].forEach((name) => {
      ...
      button.addEventListener("click", () => setView(name as View));
  ...
  function renderDocs() {
    ...
    state.docs.forEach((doc) => {
      ...
      item.innerHTML = `<img src="${doc.thumb}" alt="${doc.title}"><div><strong>${doc.title}</strong><p>${doc.pages} pages · ${doc.size}</p><span>${new Date(doc.at).toLocaleString()}</span></div>`;
  ...
  async function processed(page: Page) {
  ...
  async function importFiles(files: FileList | null) {
    ...
    const all = await Promise.all(Array.from(files).filter((f) => f.type.startsWith("image/")).map((file) => new Promise<string>((resolve, reject) => {
  ...
  async function startCamera() {
    try {
      ...
      await refs.video.play().catch(() => undefined);
  ...
  function updateCurrent() {
    const page = currentPage();
    if (!page) return;
    page.preset = refs.preset.value as Preset;
  ...
  function makePdf(canvases: HTMLCanvasElement[]) {
    ...
    const push = (idNum: number, body: Uint8Array | string) => {
  ...
  function sizeLabel(bytesCount: number) {
  ...
  async function exportPdf() {
    ...
    const title = refs.title.value.trim() || `scan-${new Date().toISOString().slice(0, 10)}`;
    ...
    state.docs.unshift({ id: id("doc"), title, at: new Date().toISOString(), pages: state.pages.length, size: sizeLabel(blob.size), thumb: canvases[0].toDataURL("image/jpeg", 0.72) });
    saveDocs();
  ...
  function init() {
    ...
    refs.title.value = `scan-${new Date().toISOString().slice(0, 10)}`;
    refs.stopCameraBtn.disabled = true;
    refs.captureBtn.disabled = true;
    ...
    refs.drawer.addEventListener("click", (event) => { if (event.target === refs.drawer) refs.drawer.classList.remove("open"); });
    ...
    refs.installBtn.addEventListener("click", () => {
      ...
      state.installPrompt.prompt().catch(() => undefined);
    });
